...
void AFFChangeDetector::print(){
  ...
  cout << "Change detector: " << std::endl;
  cout << "alpha: " << alpha << std::endl;
  cout << "Burn-in count: " << BLcount << std::endl;
  cout << "Burn-in length: " << getBL() << std::endl;
  cout << "change detected: " << getChangeDetected() << std::endl;
  //  cout << "Change points:" << getChangePoints() << std::endl;
}
...
std::tuple<std::vector<bool>, std::vector<int>, std::vector<double>> AFFChangeDetector::processVectorSave(std::vector<double> vec){
  ...
  for (int i=0; i < vec.size(); i++){
    ...
    if (changeDetected){
      cout << i << ": change detected" << std::endl;
      numChangepoints++;
  ...
  return std::make_tuple(isChangeVec, changepoints, lambdasave); 

}

//just process a vector of observations
void AFFChangeDetector::processVector(std::vector<double> vec){
  for (int i=0; i < vec.size(); i++){
    update(vec[i]);
  }
}
...
//std::vector<double> AFFChangeDetector::getChangePoints(){
//  return 
//}
